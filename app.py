...
def create_app(test_config=None):
    ...
    def index():
        ...
        standings = []
        for p in players:
            points = db.session.query(db.func.coalesce(db.func.sum(Result.points), 0)).filter(Result.player_id==p.id).scalar() or 0
            cash = db.session.query(db.func.coalesce(db.func.sum(Result.cash_delta), 0.0)).filter(Result.player_id==p.id).scalar() or 0.0
            games_played = Result.query.filter_by(player_id=p.id).distinct(Result.game_id).count()
            standings.append({
                "player": p,
                "points": int(points),
                "cash": float(cash),
    ...
    def player_new():
        ...
        return render_template("players.html", form=form, create_mode=True, players=Player.query.all())

    @app.route("/players/<int:player_id>/delete", methods=["POST"])
    @login_required
    def player_delete(player_id):
        p = Player.query.get_or_404(player_id)
        # Safeguard: cannot delete if player appears in results
        has_results = Result.query.filter_by(player_id=p.id).first()
        if has_results:
            flash("Impossibile eliminare: il giocatore ha risultati associati.", "warning")
            return redirect(url_for("players"))
        db.session.delete(p)
        db.session.commit()
    ...
    def game_new():
        form = GameCreateForm()
        form.participants.choices = [(p.id, p.name) for p in Player.query.order_by(Player.name.asc()).all()]
        if form.validate_on_submit():
            # 1) Leggi input grezzi
            buy_in = float(form.buy_in.data)
            if form.date.data:
                date = form.date.data
            else:
                date = datetime.utcnow().date()
                flash("Data non inserita: impostata automaticamente alla data odierna.", "warning")

            # Partecipanti selezionati (lista di int)
            selected = [int(p) for p in (form.participants.data or [])]

            # Ordine di arrivo (dal campo hidden popolato via drag&drop)
            ...
            if len(selected) < 2:
                flash("Seleziona almeno due partecipanti per creare una partita.", "danger")
                return render_template("game_new.html", form=form)

            if not positions:
                flash("Definisci l’ordine di arrivo (trascina i nomi).", "danger")
                return render_template("game_new.html", form=form)

            selected_set = set(selected)
            positions_set = set(positions)

            # Devono coincidere come insiemi e come cardinalità (impedisce duplicati/mancanze)
            if positions_set != selected_set or len(positions) != len(selected):
                flash("L’ordine di arrivo deve includere tutti e soli i partecipanti selezionati, senza duplicati.",
                      "danger")
                return render_template("game_new.html", form=form)
    ...
    def game_detail(game_id):
        ...
        results = (Result.query.filter_by(game_id=g.id)
                   .join(Player, Result.player_id==Player.id)
                   .order_by(Result.finish.asc()).all())
        return render_template("game_detail.html", game=g, results=results)

    @app.route("/games/<int:game_id>/delete", methods=["POST"])
    @login_required
    def game_delete(game_id):
        g = Game.query.get_or_404(game_id)
        # Cancella risultati + partita (cascade configurata sul relationship)
        db.session.delete(g)
        db.session.commit()
